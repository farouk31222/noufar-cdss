...
const requestDoctorProfile = async (token) => {
  ...
  if (!response.ok) {
    const error = new Error(data.message || "Unable to verify account status.");
...
const startDoctorRealtimeStream = () => {
  if (doctorRealtimeSource || !doctorSession?.token || typeof EventSource === "undefined") {
    return;
  }
  ...
  return `
    <div class="support-attachment-card support-attachment-card-${senderRole}">
      ...
      <div class="support-attachment-actions">
        <a href="${escapeNotificationHtml(fileUrl)}" target="_blank" rel="noopener noreferrer" aria-label="Open file" title="Open file">
        ...
        <a href="${escapeNotificationHtml(fileUrl)}" download="${escapeNotificationHtml(fileName)}" aria-label="Download file" title="Download file">
          <svg viewBox="0 0 24 24" aria-hidden="true">
            <path d="M12 4v10" />
