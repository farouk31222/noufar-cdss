...
const getPasswordState = () => {
  ...
  const hasRequiredFormat = hasLetters && hasNumbers;
...
const syncRegisterFormState = () => {
  ...
  if (passwordField) {
    if (passwordState.password.length === 0) {
      ...
      passwordField.setCustomValidity("Password must contain at least 6 characters.");
    } else if (!passwordState.hasRequiredFormat) {
      passwordField.setCustomValidity("Password must contain letters and numbers.");
    } else {
      passwordField.setCustomValidity("");
    }
  }
...
if (registerForm && registerNote) {
  ...
  registerForm.addEventListener("submit", async (event) => {
    ...
    if (!isPasswordValid) {
      setFormMessage(
        registerNote,
        "Your password must contain at least 6 characters and include letters and numbers.",
        "error",
      );
      return;
    }
