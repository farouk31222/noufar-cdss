...
const syncAccountPasswordStrengthState = () => {
  ...
  setRuleState(accountPasswordRuleNumber, passwordState.hasNumber);
...
if (changePasswordForm) {
  ...
  changePasswordForm.addEventListener("submit", async (event) => {
    ...
    if (!passwordState.hasMinLength) {
      setModalNote(changePasswordNote, "The new password must contain at least 6 characters");
      return;
    }

    if (!passwordState.hasNumber) {
      setModalNote(changePasswordNote, "The new password must include at least one number");
      return;
    }
