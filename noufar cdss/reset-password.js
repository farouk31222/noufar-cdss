...
const getPasswordState = () => {
  ...
  if (!newPassword) {
    ...
    strength = "weak";
    label = "Weak password";
  } else if (newPassword.length >= 10 && hasLower && hasUpper && hasNumber && hasSymbol) {
    strength = "strong";
  ...
  return { newPassword, confirmPassword, hasMinLength, hasNumber, matches, strength, label };
};
...
const syncPasswordState = () => {
  ...
  setRuleState(ruleNumber, state.hasNumber);
...
if (form) {
  form.addEventListener("submit", async (event) => {
    ...
    if (!state.hasMinLength) {
      setFormMessage("The new password must contain at least 6 characters", "error");
      return;
    }

    if (!state.hasNumber) {
      setFormMessage("The new password must include at least one number", "error");
      return;
    }
